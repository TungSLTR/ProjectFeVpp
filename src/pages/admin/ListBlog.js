...
const ListBlog = () => {
  ...
  const getBlog = async () => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/blog`);
  ...
  const deleteBlog = async (id) => {
    try {
     await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/blog/${id}`); 
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, blog) => (
        <Space>
          <Link href={`editblog/${blog.idblog}`}>
            <Button
              type="primary"
              size="large"
              icon={<EditOutlined />}
  ...
  return (
    <div>
      <Link href={`/admin/addblog`} className="btn btn-primary mb-2">
        Thêm Blog
      </Link>
