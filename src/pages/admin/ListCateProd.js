...
const ListCateProd = () => {
  ...
  const getCateProd = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/cateProd`);
  ...
  const deleteCateProd = async (id) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/cateProd/${id}`);
      getCateProd();
    } catch (error) {
      console.log(error);
    }
  ...
  const updateCateProd = async (data) => {
    try {
      await axios.patch(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/cateProd/${data.id}`, {
        stt: data.stt,
      });
    } catch (error) {
      console.log(error);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Link href={`editcateprod/${record.id}`}>
  ...
  return (
    <div style={{height: "100vh"}}>
      <Link href={`addcategory`} className="btn btn-primary mb-2">
        Thêm mới
      </Link>
      <DndContext
        sensors={sensors}
