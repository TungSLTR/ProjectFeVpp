...
const ListProduct = () => {
  ...
  const getProducts = async () => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/adproducts`);
    ...
    await Promise.all(
      prod.map(async (product) => {
        ...
        product.visible = product.visible === 0 ? 0 : 1;
      })
    );
    setProducts(prod);
  };
  ...
  const updateStatusProd = async (productId) => {
    try {
      ...
      const newStatus = productToUpdate.visible === 0 ? 1 : 0;
      await axios.put(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/adproducts/tt/${productId}`, {
        visible: newStatus,
      });
  ...
  const deleteProduct = async (id) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/adproducts/${id}`);
      getProducts();
    } catch (error) {
      console.log(error);
    }
      ...
      render: (_, product) => (
        <Space>
          ...
          <Button
            ...
            size="large"
            icon={
              product.visible === 0 ? <EyeOutlined /> : <EyeInvisibleOutlined />
            }
  ...
  return (
    <div className="">
      <Link href={`/admin/addProduct`} className="btn btn-primary mb-2">
        Thêm sản phẩm
      </Link>
      <Input.Search
