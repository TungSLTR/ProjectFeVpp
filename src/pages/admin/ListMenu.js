...
const ListMenu = () => {
  ...
  const deleteMenu = async (id) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/admenus/${id}`);
      getMenus();
    } catch (error) {
      console.log(error);
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, menu) => (
        <Space>
          <Link href={`/admin/editmenu/${menu.id}`}>
            <Button
              type="primary"
              size="large"
              icon={<EditOutlined />}
  ...
  return (
    <div>
      <Link href={`/admin/addmenu`} className="btn btn-primary mb-2">
        Thêm Menu
      </Link>
