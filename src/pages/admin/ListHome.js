...
const ListHome = () => {
  ...
  const getHome = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/home`);
  ...
  const deleteHome = async (id) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/home/${id}`);
      getHome();
    } catch (error) {
      console.log(error);
    }
  ...
  const updateStatus = async (id, currentStatus) => {
    try {
      ...
      await axios.put(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}admin/home/${id}`, {
        status: newStatus,
      });
      getHome();
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Link href={`/admin/edithome/${record.id}`}>
            <Button type="primary"
  ...
  return (
      ...
      <div style={{height: "100vh"}}>
        <Link href={`/admin/addhome`} className="btn btn-primary mb-2">
          Thêm mới
        </Link>
