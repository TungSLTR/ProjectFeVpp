...
export default function Contact() {
  ...
  const success = () => {
    messageApi.open({
      type: 'success',
      content: 'Cảm ơn bạn đã phản hồi',
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_dlvepq4', 'template_kwqa0wl', form.current, 'E7Y_SY4FzjYxK_yPO')
      .then((result) => {
  ...
  const fetchContact = async () => {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}contact/status`);
  ...
  return (
    <>
    <Head>
      <title>Liên hệ - Văn Phòng Phẩm Vũ Phong</title>
    </Head>
      <nav
      ...
      >
        ...
        <div class="container-fluid">
          <Link className="linkhome" href="/" title="Home">
            <svg
              aria-hidden="true"
              focusable="false"
              data-prefix="fal"
