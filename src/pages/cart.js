...
export default function Cart() {
  ...
  const deleteProduct = async (masp, makh) => {
    try {
      await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}cart/${masp}/${makh}`);
      await getUserCart();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDeleteMultiple = async () => {
    try {
      ...
      const res = await axios.delete(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}cart`, {
  ...
  const handleChangeQuanity = async (quantitycheck, masp) => {
    try {
      ...
      const res = await axios.patch(`${process.env.NEXT_PUBLIC_REACT_APP_BACKEND_URL}cart`, {
        makh,
        masp,
        quantity,
      });
      ...
      <section>
        <div className="cart-thuy container">
          {cartData.length === 0 ? (
            ...
            <div class="container mt-4 overtable">
                ...
                {user !== null ? (
                  <tbody>
                    {cartData.map((product, index) => {
                      ...
                      return (
                        <tr key={product.id}>
                          ...
                          <td>
                            {thanhTien !== 0 ? thanhTien.toLocaleString("vi-VN") : 'Liên hệ'}
                          </td>
                    ...
                    <tr>
                      ...
                      <td colspan="2">{tongcong !== 0 ? tongcong.toLocaleString("vi-VN") : "Liên hệ"}</td>
                    </tr>
                  </tbody>
                ) : (
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td colspan="">{tongcong !== 0 ? tongcong.toLocaleString("vi-VN") : "Liên hệ"}</td>
                    </tr>
                  </tbody>
                )}
              </table>
          ...
          {user !== null ? (
            <div class="d-flex justify-content-between">
              ...
              <div>
                <Link class="btn btn-primary" href="/payment">
                  Thanh toán
                </Link>
              </div>
            </div>
            ...
            <div class="d-flex justify-content-between">
              ...
              <div>
                <Link class="btn btn-primary" href="/account/login">
